  ...
  useEffect(() => {
    const fetchMurals = async () => {
      try {
        ...
        // Filter only flying apsaras murals
        const flyingApsarasMurals = response.data.filter(mural => mural.subcategory === 'flying apsaras');
        
        // Sort by rating (highest first)
        const sortedMurals = flyingApsarasMurals.sort((a, b) => b.rating - a.rating);
        ...
        <div className="murals-grid">
          {murals.length > 0 ? (
            ...
            <div className="no-results">
              No flying apsaras murals found.
            </div>
          )}
        </div>
      </div>
